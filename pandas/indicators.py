...
class UnitTest(unittest.TestCase):
    ...
    def test_moving_average(self):
        close.plot(label='data', style='--')
        for ma_type in ['DEMA', 'MAMA', 'TEMA', 'EMA', 'SMA', 'TRIMA', 'KAMA', 'T3', 'WMA']:
            s = ta.MA(close, timeperiod, getattr(ta.MA_Type, ma_type))
            pd.Series(s, name=f'{ma_type}({timeperiod})').plot()
        _hma(close, timeperiod).plot()
    ...
    def test_macd(self):
        ...
        pd.Series(macd, name='macd').plot()
        pd.Series(signal, name='signal').plot()
        pd.Series(hist, name='hist').plot()
        plt.legend(fontsize='x-small')
        plt.show()
